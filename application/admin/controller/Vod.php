...
class Vod extends Base
{
    ...
    public function info()
    {
        if (Request()->isPost()) {
            ...
            if ($vodId > 0 && is_array($contentLang)) {
                ...
                foreach ($contentLang as $langCode => $fields) {
                    ...
                    model('ContentLang')->saveFields('vod', $vodId, (string)$langCode, $fields, 'manual', 1);
                }
            }
        ...
        if (!empty($info['vod_id'])) {
            foreach ($content_lang_enabled as $langCode) {
                $content_lang_data[$langCode] = model('ContentLang')->getFields('vod', $info['vod_id'], $langCode);
            }
        }
    ...
    public function contentLangTranslate()
    {
        ...
        $result = mac_content_translate($text, $from, $to, ['content_type' => 'vod']);
        if ($result === '') {
