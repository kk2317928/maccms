...
$validators=['application/common/model/VodExt.php','application/common/util/ApiV1CanonicalResource.php','application/common/util/DuplicateReviewWorkspace.php'];
foreach($validators as $file){$s=(string)file_get_contents($root.'/'.$file);if(strpos($s,'[A-Za-z0-9]{6}')===false){fwrite(STDERR,"FAIL: {$file} lacks exact mixed-case validation.\n");exit(1);}if(strpos($s,'strtoupper')!==false){fwrite(STDERR,"FAIL: {$file} still uppercases public IDs.\n");exit(1);}}
