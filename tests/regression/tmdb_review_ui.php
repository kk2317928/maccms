...
$path = $root . '/application/common/util/TmdbReviewWorkspace.php';
if (!is_file($path)) { fwrite(STDERR, "FAIL: TmdbReviewWorkspace.php is missing.\n"); exit(1); }
require_once $path;
...
foreach (['|htmlentities', '候選', '手動 TMDB ID', '無匹配', '重新配對', '欄位差異', 'is_locked', 'data-action="select"', 'data-action="manual"', 'data-action="no_match"', 'data-action="rematch"'] as $needle) {
